...
async def start(client, message):
    sticker = None
    try:
        stick_id = "CAACAgUAAxkBAAIWbGosH-ptnYMSK_WwHgXl0hR1FFhMAALWCgACgCqoVbpYfx3qNQlCHgQ"
        try:
...
async def stream_buttons(user_id: int, file_id: str):
    if STREAM_MODE and not PREMIUM_STREAM_MODE:
        ...
        if not await db.has_premium_access(user_id):
            return [
                [InlineKeyboardButton('🚀 FAST DOWNLOAD / WATCH ONLINE 🚀', callback_data='prestream')],
                [InlineKeyboardButton('📺 VIEW AUDIO & SUBS INFO ℹ️', callback_data='prestream')]
            ]
        else:
            return [
                [InlineKeyboardButton('🚀 FAST DOWNLOAD / WATCH ONLINE 🚀', callback_data=f'generate_stream_link:{file_id}')],
